...
export default function CourseManagement({ academicYears, courses }: Props) {
  ...
  const handleAddCourse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (posterFile) {
        const fileRef = ref(storage, `course-posters/${Date.now()}_${posterFile.name}`);
        const uploadResult = await uploadBytes(fileRef, posterFile);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-start">
                      <div className="w-full md:w-48 aspect-[3/4] bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden relative group">
                        {posterPreview ? (
                          <>
